// app.js — completo: Annotazioni + Tutorial + Sync + PDF.js + Sessione
(() => {
  ...
  btnJoin?.addEventListener('click', ()=>{
    ...
    appendChat('Sessione avviata (usa Segnaling per connetterti)','sys');
  });
  ...
  function initAnnotations(){
    const canvas=document.getElementById('annoCanvas'); if(!canvas) return;
    const ctx=canvas.getContext('2d');
    const penBtn=$('#pen'), eraserBtn=$('#eraser'), clearBtn=$('#clear'), thick=$('#thick'), shareBtn=$('#btnAnnoShare');
    let drawing=false, mode='pen', last=null;

    function sizeCanvas(){ const p=canvas.parentElement; const r=p.getBoundingClientRect(); const w=Math.max(1,r.width|0), h=Math.max(1,r.height|0); if(canvas.width!==w||canvas.height!==h){ const snap=(canvas.width&&canvas.height)?ctx.getImageData(0,0,canvas.width,canvas.height):null; canvas.width=w; canvas.height=h; if(snap) try{ctx.putImageData(snap,0,0);}catch{} } }
    new ResizeObserver(sizeCanvas).observe(canvas.parentElement); window.addEventListener('load', sizeCanvas); window.addEventListener('orientationchange', sizeCanvas); setTimeout(sizeCanvas,30);

    const pt=e=>{ const r=canvas.getBoundingClientRect(); const x=(e.touches?e.touches[0].clientX:e.clientX)-r.left; const y=(e.touches?e.touches[0].clientY:e.clientY)-r.top; return {x,y}; };
    function seg(a,b){ ctx.globalCompositeOperation=(mode==='pen'?'source-over':'destination-out'); ctx.lineCap='round'; ctx.lineJoin='round'; ctx.lineWidth=+thick.value; ctx.strokeStyle='#2dd4bf'; ctx.beginPath(); ctx.moveTo(a.x,a.y); ctx.lineTo(b.x,b.y); ctx.stroke(); }
    ...
    shareBtn && (shareBtn.onclick=()=>{ try{ const data=canvas.toDataURL('image/png'); sendData({t:'annoImage',data}); appendChat('Schema inviato al remoto','sys'); }catch{} });
  ...
  $('#btnPdfJs')?.addEventListener('click', ()=>{
    ...
    if(url.startsWith('blob:')) return appendChat('PDF.js esterno non può aprire file locali (blob). Carica il PDF in /docs o usa un URL pubblico.','sys');
    docFrame.src='https://mozilla.github.io/pdf.js/web/viewer.html?file='+encodeURIComponent(url);
  ...
  $('#btnSync')?.addEventListener('click', ()=>{
    ...
    if(url.startsWith('blob:')){ appendChat('Sync richiede URL web. Carica il PDF in /docs o usa un link pubblico.', 'sys'); return; }
    sendData({t:'docOpen', url}); appendChat('Sync inviato ✓', 'sys');
  });

  // WebRTC + signaling manuale
  function createPC(){
  ...
  function setupDC(ch){
    dc=ch;
    ch.onopen = ()=>{ appendChat('(canale dati aperto)','sys'); live?.classList.remove('idle'); btnLeave && (btnLeave.disabled=false); };
    ch.onclose = ()=>{ live?.classList.add('idle'); };
    ch.onmessage = ev => { try{ const m=JSON.parse(ev.data); if(m.t==='chat') appendChat(m.text,'remote'); if(m.t==='docOpen'){ if(docFrame) docFrame.src=m.url; appendChat('Documento sincronizzato dal remoto','sys'); } }catch{} };
  ...
  const sdpBox=$('#sdpBox');
  document.getElementById('btnMakeOffer')?.addEventListener('click', async ()=>{ createPC(); const off=await pc.createOffer({offerToReceiveAudio:true,offerToReceiveVideo:true}); await pc.setLocalDescription(off); const sdp=await waitIce(pc); sdpBox.value=JSON.stringify({type:'offer',sdp:sdp.sdp}); });
  document.getElementById('btnMakeAnswer')?.addEventListener('click', async ()=>{ const offer=JSON.parse(sdpBox.value||'{}'); createPC(); await pc.setRemoteDescription(offer); const ans=await pc.createAnswer(); await pc.setLocalDescription(ans); const sdp=await waitIce(pc); sdpBox.value=JSON.stringify({type:'answer', sdp:sdp.sdp}); });
  document.getElementById('btnApplyAnswer')?.addEventListener('click', async ()=>{ const ans=JSON.parse(sdpBox.value||'{}'); if(pc) await pc.setRemoteDescription(ans); });

  // GitHub Issues (rimane invariato rispetto alla tua base)
  ...
  if (tourEl){
    ...
    const steps = [
      ...
      {sel:'.tab[data-tab=\"signal\"]', text:'Apri la tab “Segnaling” per connetterti.'},
      {sel:'#btnMakeOffer', text:'Tecnico: Genera Offerta e inviala.'},
      {sel:'#sdpBox', text:'Incolla qui Offerta/Risposta.'},
      {sel:'#btnMakeAnswer', text:'Esperto: Crea Risposta.'},
      {sel:'#btnApplyAnswer', text:'Tecnico: Applica Risposta.'},
      {sel:'#btnLaser', text:'Usa Laser e Annotazioni per guidare.'}
    ];
    let idx=0;
    ...
    function showStep(i){
      idx=i; const s=steps[idx]; const t=document.querySelector(s.sel);
      stepEl.textContent = s.text; if(t) placeHighlight(t); tourEl.classList.remove('hidden');
    }
