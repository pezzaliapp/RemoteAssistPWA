// app.js – PATCH: annotazioni robuste + tutorial + pdfjs fix + sync doc URL
(() => {
  ...
  function initAnnotations(){
    ...
    function sizeCanvasToParent(){
      ...
      if (canvas.width!==w || canvas.height!==h){
        ...
        canvas.width=w; canvas.height=h; if(snap) try{ctx.putImageData(snap,0,0);}catch{}
      }
    }
  ...
  // PDF.js: usa viewer ufficiale Mozilla (evita errore CDN)
  ...
  // Sync vista → remoto: invia l'URL attuale del documento
  ...
  // ---------- WebRTC core + GitHub Issues (ridotto) ----------
  let pc=null, dc=null;
  ...
  const tourStep = tourEl?.querySelector('.tour-step');
  ...
  const steps=[
    ...
    {sel:'#btnApplyAnswer', text:'Tecnico: “Applica Risposta” — connessione stabilita.'},
    {sel:'#btnLaser', text:'Usa il Laser per indicare i dettagli sul documento.'}
  ];
  let idx=0;
